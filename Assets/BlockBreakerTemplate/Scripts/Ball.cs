...
{
	public BlockBreakerLogic blockBreaker;		//Game logic

	[SerializeField] float speed;               //The amount of units that the ball will move each second
    [SerializeField] float maxSpeed;			//The maximum speed that the ball can travel at
	Vector2 direction;							//The Vector2 direction that the ball will move in (eg: diagonal = Vector2(1, 1))
	Rigidbody2D rig;							//The ball's Rigidbody 2D component
	...
	void FixedUpdate ()
	{
		...
		if(transform.position.x > 5 && !goingLeft)					//Is the ball at the right border and is not going left (heading towards the right border)
        {					
		...
		if(transform.position.x < -5 && goingLeft)					//Is the ball at the left border and is going left (heading towards the left border)
        {					
	...
	public void SetDirection (Vector3 target)
	{
		...
		if (blockBreaker != null)
			speed += blockBreaker.ballSpeedIncrement;   //Increases the speed of the ball by the GameManager's 'ballSpeedIncrement' value
	...
	public void ResetBall()
	{
		...
		direction = Vector2.down;				//Sets the ball's direction to go down
		StartCoroutine("ResetBallWaiter");      //Starts the 'ResetBallWaiter' coroutine to have the ball wait 1 second before moving
        blockBreaker.LiveLost();				//Calls the 'LiveLost()' function in the GameManager function
	}
